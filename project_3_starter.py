...
def generate_dollar_volume_weights(close, volume):
    ...
    dollar_volume_weights = dollar_volume_weights.apply(lambda x: x/sum(x),axis=1)
...
def calculate_dividend_weights(dividends):
    ...
    1. First calculate the cumulative sum of dividends of each stock at every date
    2. Calculate the total cumulative portfolio dividends of all stocks combined at every date 
    3. Calulcate the weights for each stock at each date by dividing the first over the second
    
    """
    print('calculating cumulative sum of each stock at each date')
    stock_dividends = dividends.apply(lambda x: x.cumsum(),axis=0)
    
    print('numerator calculated')
    
    print('calculating total portfolio dividends')
    total_portfolio_price = 0
    total_portfolio_dict = {}
    for date,val in dividends.iterrows():
        
        print('for date: {}'.format(date))
        
        print('dividends across portfolio are: \n{}'.format(val))
        
        total_div = val.cumsum()
        print('total div is: \n{}'.format(total_div))
        
        portfolio_sum = val.sum()
        
        print('portfolio sum is {}'.format(portfolio_sum))
        
        total_portfolio_price += portfolio_sum
        total_portfolio_dict[date] = total_portfolio_price
        
        print('total portfolio price is: {}'.format(total_portfolio_price))
    
    total_dividends = pd.Series(total_portfolio_dict)
    
    total_dividends = pd.DataFrame(total_dividends)
    
    assert total_dividends.index.equals(dividends.index)
    total_dividends.rename(columns={0:'portfolio_dividends'},inplace=True)
    
    print('denominator calculated')

    print('calculating weights')
    weights_list = []
    for stockdiv,portfoliodiv in zip(stock_dividends.iterrows(),total_dividends.iterrows()):
        
        print('individual stock dividends on {} are : \n{}'.format(stockdiv[0],stockdiv[1])) #Index because these are tuples
        
        print('aggregate portfolio dividends on {} are: \n{}'.format(portfoliodiv[0],portfoliodiv[1].values[0]))
        
        weights = stockdiv[1]/(portfoliodiv[1].values[0])
        
        print('individual stock weights are: \n{}'.format(weights))
        
        weights_list.append(weights)
    
    
    dividend_weights = pd.DataFrame(weights_list)
...
def calculate_cumulative_returns(returns):
    ...
    returns_agg = returns.apply(lambda x: x.sum(),axis=1)
    
    print('aggregated returns for the portfolio: \n{}'.format(returns_agg))
    
    cumulative_returns = (1+returns_agg).cumprod()
...
def tracking_error(benchmark_returns_by_date, etf_returns_by_date):
    ...
    portfolio_excess_return = etf_returns_by_date - benchmark_returns_by_date
    
    daily_tracking_error = portfolio_excess_return.std()
    
    tracking_error = np.sqrt(252)*daily_tracking_error
...
def get_optimal_weights(covariance_returns, index_weights, scale=2.0):
    ...
    print('Input shapes post assert: \n{} and {}'.format(len(covariance_returns.shape),len(index_weights.shape)))
    
    #Defining number of stocks by taking number of index weights
    m = index_weights.shape[0] 
    print('Number of stocks: {}'.format(m))
    ...
    #print("quadratic form: {}".format(portfolio_var))
    
    #Calculating Euclidean distance
    dist_from_index_weights = cvx.norm(x-index_weights)
    #print(dist_from_index_weights)
    
    #Minimizing Objective Function
    objective = cvx.Minimize(portfolio_var + scale*dist_from_index_weights)
    ...
    problem.solve()
    print('Problem solution: {}'.format(problem.status))
    x_values = x.value
...
def rebalance_portfolio(returns, index_weights, shift_size, chunk_size):
    ...
    Steps
    -------
    1. Define 'blocks' and 'iblocks' which is basically a snippet of the dataframe for a few days
    2. Define lookback_returns and lookback_weights that further shorten the data based on chunk_size
    3. Calculate covariance with get_covariance_returns
    4. Rebalance the weights on the last date of every lookback
    """
    ...
    for i in range(chunk_size,returns.shape[0],shift_size): 
        ...
        block = returns.head(i)
        
        iblock = index_weights.head(i)
        
        iblock = iblock.T 
        
        print('for iteration {}, start and end dates are: \n{} and {}'.format(i,block.index[0],block.index[block.shape[0]-1]))
        
        lookback_returns = block.tail(chunk_size)
        
        print('dates for returns and weights with only a {} day lookback are: \n{} and {}'.format(chunk_size,lookback_returns.index[0],lookback_returns.index[lookback_returns.shape[0]-1]))
        
        lookback_weights = iblock.apply(lambda x: x.tail(chunk_size),axis=1)
        
        cov = get_covariance_returns(lookback_returns)
        
        rebalance_date = lookback_weights.iloc[:,-1]
        
        optimal_weights = get_optimal_weights(cov,rebalance_date)
        
        print('weights rebalanced for: {}'.format(rebalance_date.name))
...
def get_portfolio_turnover(all_rebalance_weights, shift_size, rebalance_count, n_trading_days_in_year=252):
    ...
    for t in range(0,rebalance_count):
        index_sum = np.abs(all_rebalance_weights[t+1] - all_rebalance_weights[t])
        sum_total_turnover += index_sum.sum()
    
    rebalance_events = n_trading_days_in_year // shift_size
    
    portfolio_turnover = (sum_total_turnover/rebalance_count)*rebalance_events
